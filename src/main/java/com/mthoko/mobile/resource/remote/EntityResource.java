...
public abstract class EntityResource<T extends UniqueEntity> {
    ...
    private T selectOneFromWhere(String columns, String relation, String whereClause) {
        try {
            String select = String.format("SELECT %s FROM %s WHERE %s;", columns, relation, whereClause);
    ...
    public T findOneWhere(String whereClause) {
        return selectOneFromWhere(columnsSelectClause(true), getEntityName(), whereClause);
    }
    ...
    public List<T> findWhere(String whereClause) {
        return selectFromWhere(entityType, columnsSelectClause(true), getEntityName(), whereClause);
    }
    ...
    public int countWhere(String whereClause) {
        try {
            ...
            String sql = String.format("SELECT COUNT(*) AS %s FROM %s WHERE %s",
                    columnName, getEntityName(), whereClause);
    ...
    private <V extends UniqueEntity> List<Long> saveList(List<V> entities) {
        ...
        try {
            ...
            for (Map.Entry<Field, JoinColumn> columnEntry : EntityMapper.getJoinFields(entities.get(0).getClass()).entrySet()) {
                List<UniqueEntity> children = extractChildrenAndSetForeignKey(entities, columnEntry.getKey(), columnEntry.getValue());
                if (!children.isEmpty()) {
    ...
    private <V extends UniqueEntity> List<UniqueEntity> extractChildrenAndSetForeignKey(List<V> entities, Field key, JoinColumn value) {
    ...
    private <V extends UniqueEntity> List<Long> persistList(List<V> entities) throws SQLException {
        ...
        execSQL(sql);
        String idsSelectQuery = "SELECT id FROM " + entities.get(0).getClass().getSimpleName() + " ORDER BY id DESC LIMIT "
                + entities.size();
    ...
    public Long save(T entity) {
        ...
        if (!joinFields.isEmpty()) {
            for (Map.Entry<Field, JoinColumn> columnEntry : joinFields.entrySet()) {
                ...
                setForeignKeyOnChildrenFromParentPrimaryKey(parentClass, entity.getId(), columnEntry.getValue().targetEntity(), children);
                saveList(children);
            }
        }
        return entity.getId();
    ...
    private String getUpdateQuery(List<T> entities) {
        ...
        while (!allDescendants.isEmpty()) {
            ...
            for (UniqueEntity entity : subList) {
                sql.append(EntityMapper.getUpdateQuery(entity, ignoreFields(), leftEmbrace(), rightEmbrace())).append(";");
            }
        }
        return sql.toString();
    }
    ...
    private <V extends UniqueEntity> void loadAllDescendants(List<V> entities, Stack<List<? extends UniqueEntity>> allChildren) {
